...
public class Product {
    ...
    public void scrapeProduct(String url) {
        ...
        try {
            Document document = Jsoup.connect(productURL).userAgent(RandomUserAgent.randUserAgent()).get();

            Elements itemsHolder = document.select(".recs-item-list-padding.recs-col-items-1.recs-item-list-container");
    ...
    public ArrayList<String> findProduct(String url, int depth, boolean isAmazon) throws IOException, InterruptedException {
        ...
        Document doc = Jsoup.connect(url).userAgent(RandomUserAgent.randUserAgent()).get();
        if (!isAmazon) {
            Elements itemsHolder = doc.select(".recs-item-list-padding.recs-col-items-1.recs-item-list-container");
